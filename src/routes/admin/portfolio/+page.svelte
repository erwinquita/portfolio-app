<script>
  ...
  import PortfolioForm from '$lib/components/forms/PortfolioForm.svelte';
  ...
  async function handleCreateSubmit(formData, formDataObj) {
    ...
    if (response.ok) {
      ...
      toastStore.success('Portfolio created successfully!');
    } else {
      toastStore.error('Failed to create portfolio. Please try again.');
    }
  }
  ...
  async function handleEditSubmit(formData, formDataObj) {
    ...
    if (response.ok) {
      ...
      await invalidateAll();
      await fetchAllPortfolios(); // Refresh all portfolios for search
      toastStore.success('Portfolio updated successfully!');
    } else {
      const errorText = await response.text();
      console.error('Failed to update portfolio:', errorText);
      toastStore.error('Failed to update portfolio. Please try again.');
    }
  }
    ...
    <form method="POST" action="?/delete" use:enhance={() => {
      return async ({ result }) => {
        if (result.type === 'success') {
          ...
          await invalidateAll();
          await fetchAllPortfolios(); // Refresh all portfolios for search
          toastStore.success('Portfolio deleted successfully!');
        } else if (result.type === 'failure') {
          toastStore.error('Failed to delete portfolio. Please try again.');
        }
      };
    }}>
      ...
      <div class="dialog-actions">
        <button type="button" class="button" onclick={closeDialogs}>
          Cancel
        </button>
